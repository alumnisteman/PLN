...
export default function EditProjectPage({ params }: { params: Promise<{ uuid: string }> }) {
  ...
  if (project && !initialized) {
    setForm({
      ...
      boq_value: String(project.boq_value ?? ""),
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href={`/projects/${uuid}`} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 transition-colors">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <FormField label="Progress (%)">
              <input className={inputClass} type="number" min="0" max="100" step="0.1" placeholder="0" value={form.progress_percent}
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <Link href={`/projects/${uuid}`} className="px-5 py-2.5 border border-slate-200 text-slate-700 text-sm font-medium rounded-xl hover:bg-slate-50 transition-colors">
            Batal
          </Link>
          <button
            type="submit"
