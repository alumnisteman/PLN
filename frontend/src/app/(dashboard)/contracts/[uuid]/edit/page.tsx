...
export default function EditContractPage({ params }: { params: Promise<{ uuid: string }> }) {
  ...
  const mutation = useMutation({
    mutationFn: (data: Record<string, string>) =>
      contractService.update(uuid, {
        ...data,
        value: parseFloat(data.value) || 0,
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["contracts"] });
      qc.invalidateQueries({ queryKey: ["contract", uuid] });
  ...
  if (!contract) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <p className="text-sm text-slate-500">Kontrak tidak ditemukan.</p>
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href={`/contracts/${uuid}`} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 transition-colors">
          <ArrowLeft className="w-4 h-4" />
          Kembali
        </Link>
        <h1 className="text-xl font-bold text-slate-900">Edit Kontrak</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <FormField label="Nomor Kontrak" required>
              <input className={cn(inputClass, "bg-slate-50 text-slate-500")} value={form.number} disabled readOnly />
            </FormField>
            <FormField label="Project">
              <select className={cn(inputClass, "bg-slate-50 text-slate-500")} value={form.project_id} disabled>
                <option value="">Project tidak dapat diubah</option>
                {projects.map((p) => (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <FormField label="Ruang Lingkup Pekerjaan">
              <textarea className={textareaClass} rows={4} value={form.scope}
                onChange={(e) => set("scope", e.target.value)} />
            </FormField>
            <FormField label="Catatan">
              <textarea className={textareaClass} rows={3} value={form.notes}
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <Link href={`/contracts/${uuid}`} className="px-5 py-2.5 border border-slate-200 text-slate-700 text-sm font-medium rounded-xl hover:bg-slate-50 transition-colors">
            Batal
          </Link>
          <button
            type="submit"
